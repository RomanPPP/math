...
const m4 = {
  ...
  translate: function (m: mat4, tx: number, ty: number, tz: number): mat4 {
    return m4.multiply(m, m4.translation(tx, ty, tz));
  },

  xRotate: function (m: mat4, angleInRadians: number): mat4 {
    return m4.multiply(m, m4.xRotation(angleInRadians));
  },

  yRotate: function (m: mat4, angleInRadians: number): mat4 {
    return m4.multiply(m, m4.yRotation(angleInRadians));
  },

  zRotate: function (m: mat4, angleInRadians: number): mat4 {
    return m4.multiply(m, m4.zRotation(angleInRadians));
  },

  scale: function (m: mat4, sx: number, sy: number, sz: number): mat4 {
    return m4.multiply(m, m4.scaling(sx, sy, sz));
  },
  makeOrt: function (v: vec3): vec3 {
    const o: vec3 = [0, 0, 0];
  ...
  lookAt: function (cameraPosition: vec3, target: vec3, up: vec3): mat4 {
    const zAxis = v3.normalize(v3.diff(cameraPosition, target));
  ...
  copy: function (src: mat4): mat4 {
    return [...src];
  },

  transformPoint: function (m: mat4, v: vec3, dst?: vec3) {
    dst = dst || (new Array<number>(3) as vec3);
    const v0 = v[0];
    const v1 = v[1];
    const v2 = v[2];
    const d =
  ...
  transpose: function (m: mat4): mat4 {
    return [
      m[0],
      m[4],
      m[8],
  ...
  decompose(m: mat4) {
    let sx = v3.norm(m.slice(0, 3) as vec3);
    const sy = v3.norm(m.slice(4, 7) as vec3);
    const sz = v3.norm(m.slice(8, 11) as vec3);

    const det = this.determinate(m);
    if (det < 0) {
      sx = -sx;
    }
    const translation = new Array<number>(3) as vec3;
    const scale = new Array<number>(3) as vec3;
    const Rmatrix = [...m] as mat4;
    translation[0] = m[12];
    translation[1] = m[13];
    translation[2] = m[14];
